...
<div>
				...
				<div class="colum">
					...
					<p class="ml_15">
						<input type="text" name="member_id"
							...
							value="<%=request.getParameter("member_id")%>" <%}%> required
							placeholder="会員番号(半角英数字のみ)" pattern="^[0-9a-zA-Z]+$">
					</p>
				</div>
			</div>
			<input type="hidden" id="my_url" name="my_url" value="update" /> <input
				type="hidden" id="post_type" name="post_type" value="Search" />
	...
	<div class="member_info clearfix">
		<form
				...
				<div class="colum clearfix">
					...
					<p class="float_l ml_15">
						<input type="radio" name="sex" value="1"
							<%if (m_i.getSex() == 1) {%> checked <%}%>>男
					</p>
					<p class="float_l ml_15">
						<input type="radio" name="sex" value="2"
							<%if (m_i.getSex() == 0) {%> checked <%}%>>女
					</p>
				</div>
				<div class="colum clearfix">
					<p>■生年月日</p>
					<p class="float_l ml_15">
						<select name="birth_year" required>
							...
							<option value="<%=i%>" <%if (i == m_i.getBirth_year()) {%> selected
								<%}%>><%=i%></option>
							<%}%>
						</select>年
					</p>
					<p class="float_l">
						<select name="birth_month" required>
							...
							<%for (int i = 1; i <= 12; i++) {%>
							<option value="<%=i%>" <%if (i == m_i.getBirth_month()) {%> selected
								<%}%>><%=i%></option>
							<%}%>
						</select>月
					</p>
					<p class="float_l">
						<select name="birth_day" required>
							<option value=""></option>
							<%for (int i = 1; i <= 31; i++) {%>
							<option value="<%=i%>" <%if (i == m_i.getBirth_day()) {%> selected
								<%}%>><%=i%></option>
							<%}%>
						</select>日
					</p>
				...
				<div class="colum clearfix">
					...
					<p class="ml_15">
						<input type="tel" name="phone_number" style="width: 200px;"
							maxlength="15" value="<%=m_i.getPhone_number()%>" required
							pattern="[\d\-]*">
					</p>
				</div>
				<div class="colum">
			...
			<input type="hidden" id="post_type" name="member_id"
				value="<%=m_i.getMember_id()%>" /> <input type="hidden"
				id="post_type" name="post_type" value="Update" />
	</div>
